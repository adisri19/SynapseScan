...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (normalizedUrl.startsWith('https://www.gitlab.com/')) {
      normalizedUrl = 'https://gitlab.com/' + normalizedUrl.slice(23);
    }

    if (!normalizedUrl.startsWith('https://gitlab.com/')) {
      return NextResponse.json({ error: 'Repository URL must start with https://gitlab.com/ or be in "owner/repo" format' }, { status: 400 });
    }
    ...
    // Limit to top 35 files to prevent Vercel Serverless Function Timeout (10s)
