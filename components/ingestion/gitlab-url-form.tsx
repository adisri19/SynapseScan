...
interface GitLabUrlFormProps {
  onSubmit: (repoUrl: string) => Promise<void>;
  isLoading: boolean;
  error: string | null;
}

export function GitLabUrlForm({ onSubmit, isLoading, error }: GitLabUrlFormProps) {
  ...
  return (
    <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-6 shadow-lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="gitlabUrl" className="block text-xs font-mono uppercase tracking-wider text-slate-400 mb-2 font-bold">
