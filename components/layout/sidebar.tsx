    ...
    <aside className={`h-screen bg-[#0B0F17] border-r border-[#1F2937] flex flex-col justify-between ${sidebarWidth} transition-all duration-200 z-30`}>
      ...
      <div className="p-3 border-t border-[#1F2937] space-y-3">
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-3.5 py-2.5 text-[#10B981] hover:text-white hover:bg-[#10B981]/20 bg-[#10B981]/10 border border-[#10B981]/30 rounded-lg text-sm transition duration-150 font-semibold cursor-pointer ${sidebarCollapsed ? 'justify-center' : ''}`}
          title={sidebarCollapsed ? 'Make Another Audit' : undefined}
        >
          <svg className="w-5 h-5 shrink-0 text-[#10B981]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
          </svg>
          {!sidebarCollapsed && <span className="whitespace-nowrap">MAKE ANOTHER AUDIT</span>}
        </button>
      </div>
    </aside>
  );
